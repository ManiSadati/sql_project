...
namespace DataBaseProject
{
    class game
    {
        ...
        public static List<player> player_list(string date_time)
        {
            ...
            string query = "SELECT * FROM play where gameid='" + date_time+"'";
            DataTable ans = new DataTable();
            dbaccess.readDatathroughAdapter(query, ans);
            foreach (DataRow dataRow in ans.Rows)
            {
                int id = Convert.ToInt32(dataRow["playerid"].ToString());
                player p = player.find(id.ToString());
                LP1.Add(p);
            }
            return LP1;
        }
        public static player score_player(string date_time,string name,int score)
        {
            ...
            {
                int id = Convert.ToInt32(dataRow["playerid"].ToString());
                p = player.find(id.ToString());
                if (p.firstname == name)
                {
                    emp.playerid = dataRow["playerid"].ToString();
        ...
        public static play show_player(string date_time, string name)
        {
            List<play> LP = new List<play>();
            string query = "SELECT * FROM play where gameid='" + date_time + "'";
            DataTable ans = new DataTable();
        ...
        /*
        public static player tournoment_performance(int tourid, string playerid)
        {
            List<play> LP = new List<play>();
            string query = "SELECT AVG(ngoal) as \"ngoal\",AVG(tplayed) as \"tplayed\",AVG(npass) as \"npass\"," +
                "AVG(grade) as \"grade\",AVG(ncpass) as \"ncpass\",SUM(nscore) as \"nscore\",SUM(sscore) as \"sscore\"" +
                " FROM play where gameid in (SELECT date_time from game where tourid='" + Convert.ToString(tourid) + "' ) and" +
                " playerid='" + playerid + "'";
            DataTable ans = new DataTable();
            dbaccess.readDatathroughAdapter(query, ans);
            MessageBox.Show("IT IS DONEE ans.row.count " + ans.Rows.Count.ToString());
            play emp = new play();
            player p = new player();
            if (ans.Rows.Count > 0)
            {
        ...
        public static play tournoment_performance(int tourid, string playerid)
        {
            List<play> LP = new List<play>();
            MessageBox.Show(tourid.ToString());
            MessageBox.Show(playerid);
            string query = "SELECT AVG(ngoal) as ngoal,AVG(tplayed) as tplayed,AVG(npass) as npass,AVG(grade) as grade,AVG(ncpass) as ncpass,SUM(nscore) as nscore,SUM(sscore) as sscore FROM play where gameid in (SELECT date_time from game where tourid='" + Convert.ToString(tourid) + "' ) and playerid='" + playerid + "'";
            DataTable ans = new DataTable();
            dbaccess.readDatathroughAdapter(query, ans);
            play emp = new play();
            player p = new player();
            if (ans.Rows.Count > 0)
            {
                emp.ngoal = Convert.ToInt32(Convert.ToDouble(ans.Rows[0]["ngoal"].ToString()));
                emp.npass = Convert.ToInt32(Convert.ToDouble(ans.Rows[0]["npass"].ToString()));
                emp.ncpass = Convert.ToInt32(Convert.ToDouble(ans.Rows[0]["ncpass"].ToString()));
                emp.tplayed = Convert.ToInt32(Convert.ToDouble(ans.Rows[0]["tplayed"].ToString()));
                emp.grade = Convert.ToInt32(Convert.ToDouble(ans.Rows[0]["grade"].ToString()));
                emp.nscore = Convert.ToInt32(ans.Rows[0]["nscore"].ToString());
                emp.sscore = Convert.ToInt32(ans.Rows[0]["sscore"].ToString());
            }
            return emp;
